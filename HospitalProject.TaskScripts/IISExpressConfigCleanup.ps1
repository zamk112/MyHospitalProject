...
try
{
    ...
    Write-Host "Beginning site config deletion for $($WebsiteName)" -ForegroundColor Yellow
    $appCmdCommand = "delete site `"$($WebsiteName)`" /apphostconfig:`"$($IISExpressWorkspaceConfigDirectory)\applicationhost.config`""
    ...
    Write-Host "Beginning app config deletion" -ForegroundColor Yellow
    ...
    $AppPoolConfig.ForEach{
        $appCmdCommand = "delete apppool $($_) /apphostconfig:`"$($IISExpressWorkspaceConfigDirectory)\applicationhost.config`""
        $output = Invoke-Expression $(" & '" + $IISExpressAppCmdPath + "' " + $appCmdCommand)
